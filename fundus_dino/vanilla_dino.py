...
def train_dino(args):
    ...
    dataset = FundusMaskDataset(image_root=image_root, transform=transform)
...
class DINOLoss(nn.Module):
    def __init__(self, out_dim, ncrops, warmup_teacher_temp, teacher_temp,
                 ...
                 center_momentum=0.9,teacher_mom = 0.999):
        super().__init__()
        self.student_temp = student_temp
        self.center_momentum = center_momentum
        self.ncrops = ncrops
        self.epoch = 0
        self.teacher_mom = teacher_mom
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale=(0.4, 1.), local_crops_scale=(0.05, 0.4), local_crops_number=5):
        ...
        self.teacher_transform = transforms.Compose([
            transforms.RandomResizedCrop(224, scale=self.global_crops_scale, ratio=(1.0, 1.0), interpolation=Image.BICUBIC),
            flip_and_color_jitter,
            fixed_rotation,
            utils.GaussianBlur(0.5,radius_min=0.1, radius_max=1.0),
            normalize,
        ])
        # second global crop
        self.student_transform = transforms.Compose([
            transforms.RandomResizedCrop(96, scale=self.local_crops_scale, ratio=(1.0, 1.0), interpolation=Image.BICUBIC),
            fixed_rotation,
            flip_and_color_jitter,
            utils.GaussianBlur(0.5,radius_min=0.1, radius_max=1.0),
            normalize,
    ...
    def __call__(self, image):
        crops = []
        for i in range(self.local_crops_number + 2):
            if (i < 2):
                crops.append(self.teacher_transform(image))
            else:
                crops.append(self.student_transform(image))                
        return crops
